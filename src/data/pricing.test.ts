...
describe("priceFor — 3XL/4XL surcharge", () => {
  ...
  it("does not apply any surcharge at XXL", () => {
    expect(priceFor("2026/27", "XXL", "fan")).toBe(95);
    expect(priceFor("2026/27", "XXL", "player")).toBe(110);
    expect(priceFor("2021/22", "XXL", "fan")).toBe(135);
  });
...
describe("bitDepositFor — cash-on-delivery deposit tiers", () => {
  it("is 35 below 150", () => {
    expect(bitDepositFor(0)).toBe(35);
    expect(bitDepositFor(149)).toBe(35);
  });

  it("is 40 from 150 up to (not including) 220", () => {
    expect(bitDepositFor(150)).toBe(40);
    expect(bitDepositFor(219)).toBe(40);
  });

  it("is 50 at 220 and above", () => {
    expect(bitDepositFor(220)).toBe(50);
    expect(bitDepositFor(1000)).toBe(50);
  });
});
