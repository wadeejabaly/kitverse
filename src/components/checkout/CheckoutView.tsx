...
export function CheckoutView({
  ...
  // Shipping needs a region — nothing until the shopper has picked one.
  ...
  function setField(field: FieldName, value: string) {
    // `region` is a narrower union than `string` (DeliveryRegion | ""); every
    // value this is ever called with for that field comes from the <select>
    // options below, which only ever emit a valid DeliveryRegion or "".
    ...
    <div className="grid gap-12 pt-8 pb-16 wide:grid-cols-[1fr_380px] wide:gap-16">
      ...
      <section>
        ...
        <div className="flex flex-col gap-5">
          ...
          {/* Delivery region — drives the shipping cost shown below. A select,
              not inferred from the free-text city above: guessing a region
              from a hand-typed city is exactly the kind of silent mismatch
              that costs the store money on every order shipped to the wrong
              tier. */}
          <div>
            <label htmlFor="checkout-region" className="mono-eyebrow mb-2 block text-ink-soft">
              {t("fieldRegion")}
            </label>
            <select
              id="checkout-region"
