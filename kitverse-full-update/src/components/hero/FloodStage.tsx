...
/**
 * <HeroD/>'s stage: one shirt lit by its own radial glow, with a small
 * corner chip naming it and its price, slowly exchanged for the next shirt.
 *
 * Structurally this is the same safety story as Variant A's <ShirtStage/>:
 * every layer is server-rendered, the first shirt sits at full opacity and
 * the rest at zero, and the timer only ever changes which layer is opaque —
 * so a failed hydration, a reader with no JS and a reader who has asked for
 * reduced motion all land on "the first shirt is on screen", which is also
 * the LCP element. What changes for the Floodlight surface is cosmetic: a
 * transparent stage instead of a white tile, a glow behind the shirt instead
 * of a plain background, and the caption folded into an overlaid chip
 * instead of a row underneath — matching the chosen mockup.
 */
      ...
      {shirts.map((shirt, index) => {
        ...
        return (
          ...
          >
            ...
            />
            {/* The chip is part of the SAME crossfading layer as its shirt
                (not a separate stacked row like Variant A's caption) so it
                can only ever be on screen with the shirt it names. */}
