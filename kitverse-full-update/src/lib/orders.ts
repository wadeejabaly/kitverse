 ...
 *   - the price comes from the product's `season` (Retro vs Fan/Player), not
 *     the client's say-so.
 *
 * The addon flags follow the sanitized value: an empty name after sanitising
 * is no personalisation, so it is not charged for.
 *
 * `region` drives shipping via shippingFor() — required, because every order
 * (card or cash-on-delivery) now ships at a region-specific rate rather than
 * one flat figure.
 */
